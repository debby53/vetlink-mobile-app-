  ...
  const validateStep = (step: number) => {
    switch (step) {
      case 1: // Role
        return true;
      case 2: // Personal
        if (!formData.name) return t('fillAllFields');
        if (!formData.email) return t('fillAllFields');
        // Phone optional
        return true;
      case 3: // Location
        // Required for Vets/CAHWs based on old logic, but generally good for all
        if (!locationState.id) return t('fillAllFields');
        return true;
  ...
  const renderStep = () => {
    switch (currentStep) {
        ...
        return (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="grid grid-cols-1 gap-4">
              {[
                { id: 'farmer', label: t('roleFarmer'), icon: Sprout, desc: t('roleFarmerDescription') },
                { id: 'veterinarian', label: t('roleVeterinarian'), icon: Stethoscope, desc: t('roleVeterinarianDescription') },
                { id: 'cahw', label: t('roleCAHW'), icon: Briefcase, desc: t('roleCAHWDescription') },
              ].map((item) => {
      ...
      case 2:
        return (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">{t('phoneOptional')}</label>
              <div className="flex">
              ...
              <p className="text-xs text-slate-500 mt-1">{t('phoneRequired')}</p>
            </div>
          </div>
        );
      ...
      case 3:
        return (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-xl font-semibold mb-4">{t('whereLocated')}</h2>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <LocationSelector
                onLocationSelect={handleLocationSelect}
                label={t('searchSector')}
              />
            </div>
            {locationState.sector && (
              <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 p-3 rounded-lg">
                <MapPin className="w-4 h-4" />
                {t('selectedLocation')} {locationState.sector}, {locationState.district}
              </div>
            )}
          </div>
        );
      ...
      case 4:
        if (role === 'veterinarian') {
          return (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <h2 className="text-xl font-semibold mb-4">{t('professionalInfo')}</h2>
              <div>
      ...
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row min-h-[600px]">
        ...
        <div className="bg-primary p-8 md:w-1/3 text-white flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-3xl font-bold mb-4">{t('joinOurCommunity')}</h1>
            <p className="text-primary-foreground/80">{t('signupSubtitle')}</p>
          </div>
