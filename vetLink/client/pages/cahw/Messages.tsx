...
export default function CAHWMessages() {
  const { user } = useAuth();
  ...
  const loadDirectFarmerChat = async (farmerId: number) => {
    if (!user?.id) return;
    try {
      const farmerData = await userAPI.getUserById(farmerId);
      setSelectedChatUser(farmerData);
      const msgs = await messageAPI.getConversation(user.id, farmerId);
      const sortedMsgs = (msgs || []).sort((a, b) => {
        const dateA = parseDate(a.createdAt).getTime();
        const dateB = parseDate(b.createdAt).getTime();
        return dateA - dateB;
  ...
  useEffect(() => {
    ...
    if (farmerId) {
      ...
      setSelectedChat(farmerIdNum);
      loadDirectFarmerChat(farmerIdNum);
      console.log('Auto-selected farmer:', farmerIdNum);
    }
  }, [searchParams]);
  ...
  const selectUserToMessage = (selectedUser: any) => {
    ...
    setSearchUsers([]);
    loadDirectFarmerChat(selectedUser.id);
  };
